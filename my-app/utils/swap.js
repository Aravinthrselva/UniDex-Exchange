...
export const getAmountOfTokensReceievedFromSwap = async ( 
    _swapAmountWei,
    provider,
    ethSelected,
    ethBalance,
...
export const swapTokens = async(
  ...
  // User has to approve `swapAmountWei` for the contract because `Crypto Dev` token  is an ERC20   
 ...
 // send back `tokenToBeReceivedAfterSwap` amount of `Eth` to the user
        tx = await tokenContract.cryptoDevTokenToEth(
            swapAmountWei,
            tokenToBeReceivedAfterSwap
        );      
        await tx.wait(); 
